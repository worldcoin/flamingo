//! End-to-end tests for the v2 WebSocket client against a stub host.
...
#[tokio::test]
async fn sends_custom_headers_on_the_v2_upgrade() {
    let listener = TcpListener::bind(SocketAddr::from((Ipv4Addr::LOCALHOST, 0)))
        .await
        .expect("should bind an ephemeral port");
    let address = listener
    ...
    let server = tokio::spawn(async move {
        ...
        assert!(request.starts_with("GET /v2/matches HTTP/1.1\r\n"));
        assert!(
            request
                .to_ascii_lowercase()
