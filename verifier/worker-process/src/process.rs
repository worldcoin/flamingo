...
impl Worker {
    ...
    /// Call from a single-threaded bootstrap before generating keys. Waits for model readiness.
    ...
    /// Initialization completed before this owner was returned.
...
fn kill(pid: libc::pid_t) {
    ...
    if unsafe { libc::kill(pid, libc::SIGKILL) } != 0 {
        ...
        if error.raw_os_error() != Some(libc::ESRCH) {
            tracing::error!(dependency = "biometric_worker", pid, %error, "worker kill failed");
        }
    }
}
