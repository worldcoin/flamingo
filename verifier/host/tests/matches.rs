//! Real-socket tests for the `/v1/matches` WebSocket session.
//!
...
async fn connect(address: SocketAddr) -> Client {
    let (socket, _) = connect_async(format!("ws://{address}/v1/matches"))
        .await
...
async fn capacity_refuses_with_503_and_releases_on_close() {
    ...
    let error = connect_async(format!("ws://{address}/v1/matches"))
        .await
    ...
    for attempt in 0..100 {
        if connect_async(format!("ws://{address}/v1/matches"))
            .await
            .is_ok()
        {
            return;
