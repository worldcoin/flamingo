...
impl Config {
    pub fn from_env() -> Result<Self> {
        let environment = env::var("BUNDLE_ENVIRONMENT").context("missing BUNDLE_ENVIRONMENT")?;
        let release_id = env::var("BUNDLE_RELEASE_ID").context("missing BUNDLE_RELEASE_ID")?;
        let artifact_uri =
            env::var("BUNDLE_ARTIFACT_URI").context("missing BUNDLE_ARTIFACT_URI")?;
        let artifact_sha256 =
            env::var("BUNDLE_ARTIFACT_SHA256").context("missing BUNDLE_ARTIFACT_SHA256")?;
...
pub fn ready_file() -> PathBuf {
    setting("BUNDLE_READY_FILE", "/run/flamingo/ready").into()
}
...
fn validate_pin(environment: &str, release: &str, uri: &str, digest: &str) -> Result<()> {
        ...
        "invalid bundle environment"
    );

    let version = release
        .strip_prefix("biometric-engines-worker-v")
        .context("invalid bundle release id")?;
    ensure!(
        ...
        "invalid bundle release version"
    );
