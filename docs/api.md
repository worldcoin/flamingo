...
The client verifies the assignment, seals the match to that key, and verifies the result and its claims without a second round trip for ordering. `FlamingoVerifierClient::connect` returns a session whose `request_match` runs the exchange; the session owns the socket and the verified assignment, so a match cannot be sent over a connection whose assignment was not verified. On native targets, authenticated gateways can use `build_request()`, add an `Authorization` header with `with_header`, then pass the builder to `connect_with()`. Browser WebSockets cannot set custom upgrade headers; browser callers must use a gateway authentication mechanism supported by the browser, such as cookies.
